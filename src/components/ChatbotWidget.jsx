...
import { BotMessageSquare, LoaderCircle, MessageCircle, SendHorizonal, Sparkles, X } from "lucide-react";
      ...
      <div className="pointer-events-none fixed inset-x-4 bottom-4 z-50 flex justify-end sm:inset-x-auto sm:right-5 sm:bottom-5">
        <div className="pointer-events-auto flex w-full max-w-[24rem] flex-col items-end gap-3">
          <AnimatePresence initial={false}>
            {isOpen ? (
              <motion.section
                ...
                className="w-full overflow-hidden rounded-[1.6rem] border border-white/12 bg-[color:color-mix(in_srgb,var(--card)_90%,black_10%)] shadow-[0_24px_60px_rgba(0,0,0,0.28)] backdrop-blur-xl"
              >
                <motion.div
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <Button
                      ...
                      variant="ghost"
                      size="icon-xs"
                      className="bg-transparent text-muted-foreground hover:bg-white/8"
                ...
                <motion.div variants={contentVariants} className="px-4 pb-4 pt-3">
                  <Button
                    type="button"
                    variant="outline"
                    className="mb-3 h-auto w-full justify-start rounded-2xl border-white/10 bg-white/4 px-3 py-2 text-left text-xs text-muted-foreground hover:bg-white/7 text-wrap"
                    onClick={() => submitMessage(suggestedPrompt)}
                    disabled={isLoading}
                  >
                    <BotMessageSquare className="mt-0.5 size-3.5 shrink-0 text-primary " />
                      ...
                      <div className="space-y-3">
                        ...
                        {isLoading ? (
                          <motion.div
                            ...
                            className="flex max-w-[88%] items-center gap-2 rounded-2xl bg-white/6 px-3 py-2 text-sm text-muted-foreground"
                          >
                  ...
                  {errorMessage ? (
                    <motion.p
                      ...
                      animate={prefersReducedMotion ? undefined : { opacity: 1, y: 0 }}
                      className="mt-3 rounded-2xl border border-destructive/30 bg-destructive/10 px-3 py-2 text-xs text-destructive"
                    >
                      {errorMessage}
                    </motion.p>
                  ) : null}
                  ...
                  >
                    <Input
                      ...
                      className="h-11 rounded-2xl border-white/10 bg-white/5 px-3 text-sm"
                      disabled={isLoading}
                    />
                    <Button
                      type="submit"
          ...
          <Button
            type="button"
            size="icon-lg"
            className="shadow-[0_18px_38px_rgba(255,102,0,0.35)]"
